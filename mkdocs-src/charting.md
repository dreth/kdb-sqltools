...
7. Press `Render`.
8. Drag to zoom x; use `Shift`+drag, Pan left/right, or `ArrowLeft`/`ArrowRight` to pan x; use `Home`, double-click, or Reset to restore the full range. `Refine view` explicitly resamples the current viewport.
...
uPlot powers the built-in chart. Supported interactions are cursor/crosshair tooltip values, x-only drag zoom, `Shift`+drag x pan, 20% button/arrow-key pan, explicit viewport refinement, reset, legend labels with live values, legend series toggling, splitter resizing, and PNG export. Zoom and pan use one settled-viewport coordinator. For the same clamped absolute range, pan completion uses the exact unchanged zoom range-loading/resampling decision; mousemove sends no source request, y stays automatic, and a nested zoom composes from the panned absolute viewport. Duplicate notifications are debounced/deduplicated, while reconstruction, resize, settings, and series-visibility rerenders do not recursively request data.
...
Settled viewport refinement scans the same guarded full source result and resamples only the selected absolute x range. A completed pan enters the same unchanged decision as a completed zoom for that range; it does not add another sampling policy. Requests are normally debounced for about 450 ms. Distinct queued ranges and in-flight supersession retain the existing zoom lifecycle so older responses cannot erase newer intent. Identical scale notifications are deduplicated, and scale changes caused by response reconstruction, resize, settings, or hide/show rerenders are suppressed. `Refine view` explicitly applies that same current-viewport path. The webview does not rescan on mousemove.
