...
All chart types keep auto-thinned readable x-axis labels, cursor tooltips, selectable Zoom/Pan drag modes, `Shift`+drag x pan, a full-range navigator, `ArrowLeft`/`ArrowRight` keyboard pan, `Home`/double-click/reset, splitter resizing, legend toggling, and PNG export after render. The navigator always retains the immutable full sample while its movable and resizable window controls the visible x range. Y remains automatic. Identical scale notifications are deduplicated, mousemove feedback does not request source data, and programmatic reconstruction, resize, settings, and series-visibility rerenders do not issue recursive refinement requests.

After zoom, pan, or navigator movement settles, a viewport with at least 3,000 already-sampled x values stays local; a sparser viewport is automatically rebuilt from the retained source. A rebuilt range with fewer than 3,000 eligible rows renders every available row without upsampling. From 3,000 through 7,000 it keeps the available density; above 7,000 it uses the chart type's reduction model to return about 7,000 points. `Reset zoom` invalidates stale in-flight refinement responses and restores the immutable original full sample/domain without another q/backend request. Candlestick tooltips show open/high/low/close values, and exported PNGs include the rendered candle geometry. Changing chart type or another control updates visible fields and validation without removing the old rendered chart; press `Render` to apply the new settings. Successful selections are restored only when the visible-column signature and the type-specific roles remain valid, so older generic selections fall back safely.
...
Copy formats: CSV, TSV, JSON, NDJSON, HTML, Markdown. Right-click Copy in the table viewport uses the same selected range and copy settings as `Ctrl+C` / `Cmd+C`. Export formats: CSV, XLSX, TSV, JSON, NDJSON, HTML, Markdown. XLSX export writes a real `.xlsx` workbook and rejects output beyond Excel's sheet limits; XLSX remains export-only and cannot be copied. Duplicate JSON/NDJSON column names receive deterministic suffixes instead of overwriting values. Large copy/export actions prompt before materializing output; the confirmation cell threshold is configurable from Settings -> Preferences or `kdb-sqltools.results.copyExportConfirmCellThreshold`, while fixed clipboard, text, cell, worksheet, and file-size limits always remain in force. The row-number/header copy and export options are enabled by default and persist globally from the panel settings menu.

Visible cells are bounded to 64 KiB of display text and search inspects at most 4 KiB per cell without changing the full values used by copy/export. Keyed-table key columns receive a theme-aware tint. Optional bounded column summaries and safe qText syntax highlighting/display formatting are available from Settings; qText copy/export always preserves the original text.
